...
function manualDispatchChangeEvent(nativeEvent) {
  ...
  // component don't work properly in conjunction with event bubbling because
...
var EnterLeaveEventPlugin = {
  ...
  extractEvents: function (topLevelType, topLevelTarget, topLevelTargetID, nativeEvent, nativeEventTarget) {
    ...
    if (from === to) {
      // Nothing pertains to our managed component.
      return null;
    }
 ...
 * this outside of the ReactDOM controlled form component.
 */
...
/**
 * Composite component are higher-level component that compose other composite
 * or native component.
 *
...
function validateTypeDef(Constructor, typeDef, location) {
  for (var propName in typeDef) {
    if (typeDef.hasOwnProperty(propName)) {
      // use a warning instead of an invariant so component
...
/**
 * Module for creating composite component.
 *
 * @class ReactClass
 */
var ReactClass = {
 ...
 *
 * Most React component have pure render functions.
 *
 * Example:
 *
 ...
 * differences. Only mixin to component which have simple props and state, or
 ...
 * used to enforce the order in which `ReactUpdates` updates dirty component.
 *
 * @private
 */
var nextMountID = 1;
...
var ReactCompositeComponentMixin = {
  ...
  mountComponent: function (rootID, transaction, context) {
    ...
    if ("development" !== 'production') {
      ...
      if (inst.render == null) {
        ...
        // and stateless component, but not ES6 classes that don't extend
  ...
  unmountComponent: function () {
    ...
    // Some existing component rely on inst.props even after they've been
  ...
  attachRef: function (ref, component) {
    ...
    !(inst != null) ? "development" !== 'production' ? invariant(false, 'Stateless function component cannot have refs.') : invariant(false) : undefined;
    var publicComponentInstance = component.getPublicInstance();
    if ("development" !== 'production') {
      var componentName = component && component.getName ? component.getName() : 'a component';
      "development" !== 'production' ? warning(publicComponentInstance != null, 'Stateless function component cannot be given refs ' + '(See ref "%s" in %s created by %s). ' + 'Attempts to access this ref will fail.', ref, componentName, this.getName()) : undefined;
    }
...
function processChildContextDev(context, inst) {
  // Pass down our tag name to child component for validation purposes
 ...
 * React component. It accepts event listeners and DOM properties that are
  ...
  unmountComponent: function () {
    switch (this._tag) {
      ...
      case 'body':
         ...
         */
        !false ? "development" !== 'production' ? invariant(false, '<%s> tried to unmount. Because of cross-browser quirks it is ' + 'impossible to unmount some top-level component (eg <html>, ' + '<head>, and <body>) reliably and efficiently. To fix this, have a ' + 'single top-level component that never unmounts render these ' + 'elements.', this._tag) : invariant(false) : undefined;
        break;
    }
...
function _handleChange(event) {
  ...
  // is important when using controlled component within layers:
...
/**
 * Text nodes violate a couple assumptions that React makes about component:
 *
 ...
 *
 * TODO: Investigate representing React component in the DOM with text nodes.
 *
...
var ReactDefaultBatchingStrategy = {
   ...
   * and friends are batched such that component aren't updated unnecessarily.
   */
  ...
  measure: function (moduleName, fnName, func) {
    return function () {
      ...
      if (fnName === '_renderNewRootComponent' || fnName === 'flushBatchedUpdates') {
        ...
        // component that rendered and the DOM operations that actually
...
function getInclusiveSummary(measurements, onlyClean) {
  ...
  for (var i = 0; i < measurements.length; i++) {
    ...
    for (var id in allIDs) {
      ...
      // Inclusive time is not useful for many component without knowing where
...
function getUnchangedComponents(measurement) {
  // For a given reconcile, look at which component did not actually
...
var ReactElementValidator = {
  ...
  createElement: function (type, props, children) {
    ...
    "development" !== 'production' ? warning(validType, 'React.createElement: type should not be null, undefined, boolean, or ' + 'number. It should be a string (for DOM elements) or a ReactClass ' + '(for composite component).%s', getDeclarationErrorAddendum()) : undefined;
...
// This registry keeps track of the React IDs of the component that rendered to
...
function handleTopLevelWithoutPath(bookKeeping) {
  ...
  // Loop through the hierarchy, in case there's any nested component.
 ...
 * component. This is now being replaced with an opaque data structure.
...
var ReactFragment = {
  ...
  create: function (object) {
    ...
    !(object.nodeType !== 1) ? "development" !== 'production' ? invariant(false, 'React.addons.createFragment(...): Encountered an invalid child; DOM ' + 'elements are not valid children of React component.') : invariant(false) : undefined;
...
/**
 * Creates a DOM ID prefix to use when mounting React component.
 *
...
/**
 * Manages the IDs assigned to DOM representations of React component. This
 ...
 * composites instead of having to worry about different types of component
 * here.
 */
...
var ReactMount = {
  ...
  /**
   * This is a hook provided to support rendering React component while
  ...
  _renderSubtreeIntoContainer: function (parentComponent, nextElement, container, callback) {
    ...
    "development" !== 'production' ? warning(!container || !container.tagName || container.tagName.toUpperCase() !== 'BODY', 'render(): Rendering component directly into document.body is ' + 'discouraged, since its children are often manipulated by third-party ' + 'scripts and browser extensions. This may lead to subtle ' + 'reconciliation issues. Try rendering into a container element created ' + 'for your app.') : undefined;
    ...
    if ("development" !== 'production') {
      "development" !== 'production' ? warning(!containerHasNonRootReactChild, 'render(...): Replacing React-rendered children with a new root ' + 'component. If you intended to update the children of this node, ' + 'you should instead have the existing children update their state ' + 'and render the new component instead of calling ReactDOM.render.') : undefined;
  ...
  /**
   * Registers a container node into which React component will be rendered.
...
/**
 * ReactOwners are capable of storing references to owned component.
 *
 * All component are capable of //being// referenced by owner component, but
 * only ReactOwner component are capable of //referencing// owned component.
 ...
 * supplied to React component. Example usage:
 *
...
ReactRef.shouldUpdateRefs = function (prevElement, nextElement) {
  ...
  return(
    // This has a few false positives w/r/t empty component.
...
var ReactTestUtils = {
  ...
  /**
   * Finds all instance of component in the rendered tree that are DOM
   * component with the class name matching `className`.
  ...
  /**
   * Finds all instance of component in the rendered tree that are DOM
   * component with the tag name matching `tagName`.
   * @return {array} an array of all the matches.
   */
  ...
  /**
   * Finds all instances of component with type equal to `componentType`.
   * @return {array} an array of all the matches.
   */
...
ReactShallowRenderer.prototype.render = function (element, context) {
  ...
  !(typeof element.type !== 'string') ? "development" !== 'production' ? invariant(false, 'ReactShallowRenderer render(): Shallow rendering works only with custom ' + 'component, not primitives (%s). Instead of calling `.render(el)` and ' + 'inspecting the rendered output, look at `el.props` directly instead.', element.type) : invariant(false) : undefined;
...
function runBatchedUpdates(transaction) {
  ...
  !(len === dirtyComponents.length) ? "development" !== 'production' ? invariant(false, 'Expected flush transaction\'s stored dirty-component length (%s) to ' + 'match dirty-component array length (%s).', len, dirtyComponents.length) : invariant(false) : undefined;
...
function enqueueUpdate(component) {
  ...
  // destruction of top-level component is guarded in ReactMount.)
...
function instantiateReactComponent(node) {
  ...
  if (node === null || node === false) {
    ...
    var element = node;
    !(element && (typeof element.type === 'function' || typeof element.type === 'string')) ? "development" !== 'production' ? invariant(false, 'Element type is invalid: expected a string (for built-in component) ' + 'or a class/function (for composite component) but got: %s.%s', element.type == null ? element.type : typeof element.type, getDeclarationErrorAddendum(element._owner)) : invariant(false) : undefined;

    // Special case string values
    if (typeof element.type === 'string') {
      instance = ReactNativeComponent.createInternalComponent(element);
    } else if (isInternalComponentType(element.type)) {
      // This is temporarily available for custom component that are not string
  ...
  // respectively. Instead of using expandos on component, we should be
