...
(function($) {
	...
	$.fn.bcalendar = function(option) {
		...
		//日期和月的函数
		var __SCOLLEVENTTEMP = "<DIV style=\"WIDTH:${width};top:${top};left:${left};height:${height}px;box-sizing: border-box;border-bottom: rgba(45, 143, 249, 0.2) solid 1px;border-top:rgba(45, 143, 249, 0.2) solid 1px;border-left: rgba(45, 143, 249, 0.2) solid 1px; border-right: rgba(45, 143, 249, 0.2) solid 1px;background: rgba(45, 143, 249, 0.2);2d8ff9\" title=\"${title}\"  class=\"chip chip${i} ${drag}\"><div class=\"dhdV\" style=\"display:none\">${data}</div><DL><DT>${starttime} – ${endtime} ${icon}</DT><DD><SPAN>${content}</SPAN></DD></DL></DIV>";
		...
		function BuildDaysAndWeekView(startday, l, events, config) {
			...
			var html = [];
			html.push("<div class=\"wktopcontainer dvwkcontaienr\">");
			...
			html.push("<div class=\"scolltimeevent dvtec\"><table style=\"table-layout: fixed;", jQuery.browser.msie ? "" : "width:100%", "\" cellspacing=\"0\" cellpadding=\"0\"><tbody><tr><td>");
			html.push("<table style=\"height:1000px\" class=\"tg-timedevents tgTable\" cellspacing=\"0\" cellpadding=\"0\"><tbody>");
			...
			for(var i = 0; i < l; i++) {
				...
				if(de.length > 0) { //存在日程
					...
					for(var y = 0; y < t; y++) {
						...
						x.aQ = Math.min(1 - x.left, x.width + 0 * p); //width的偏移
					}
					de = null;
					deA[i] = k;
				}
		...
		function BuildWT(ht, dayarrs, events, dMax) {
			...
			//onclick=\"javascript:FunProxy('rowhandler',event,this);\"
			ht.push("><div class=\"weekViewAllDaywk\" ><table class=\"st-grid\" cellpadding=\"0\" cellspacing=\"0\"><tbody>");
		...
		function BuildDayScollEventContainer(ht, dayarrs, events) {
			//1:
			ht.push("<tr>");
			ht.push("<td style='width:50px;'></td>");
			ht.push("<td");
			if(dayarrs.length > 1) {
				ht.push(" colSpan='", dayarrs.length, "'");
			}
			ht.push("><div class=\"tg-spanningwrapper tgspanningwrapper\"><div style=\"font-size: 20px\" class=\"tg-hourmarkers\">");
			for(var i = 0; i < 25; i++) {
			...
			//2:
			ht.push("<tr>");
			ht.push("<td style=\"width: 50px\" class=\"tg-times\">");
			...
			ht.push("<div style=\"height:41px\" class=\"tg-time\">", "24时", "</div>");
			ht.push("</td>");
			...
			for(var i = 0; i < l; i++) {
				...
				// Today
				if(istoday) {
					ht.push("<div style=\"margin-bottom: -1000px; height:1000px\" class=\"tg-today\">&nbsp;</div>");
				}
				//var eventC = $(eventWrap);
				//onclick=\"javascript:FunProxy('rowhandler',event,this);\"
				ht.push("<div  style=\"margin-bottom: -1000px; height: 1000px\" id='tgCol", i, "' class=\"tg-col-eventwrapper\">");
				...
				if(istoday) {
					//					var mhh = mHg + 4;
					var mhh = mHg - 20;
					ht.push("<div class=\"point\" style=\"left:0px;top:", mhh-2, "px\"></div><div class=\"tg-hourmarker tg-nowmarker tgnowmarker\" \" style=\"left:0px;top:", mhh, "px\"></div>");
				}
				ht.push("</div>");
				ht.push("</td>");
			}
